...
from dataset import DatasetRegistry, register_coco, register_balloon, register_objtiny, register_obj365
...
def do_evaluate(pred_config, output_file):
    ...
    for dataset in cfg.DATA.VAL:
        ...
        output = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'val_output.json')
        DatasetRegistry.get(dataset).eval_inference_results(all_results, output)


def do_test(pred_config, output_file):
    num_tower = max(cfg.TRAIN.NUM_GPUS, 1)
    graph_funcs = MultiTowerOfflinePredictor(
        pred_config, list(range(num_tower))).get_predictors()
    ...
    for dataset in cfg.DATA.TEST:
        ...
        dataflows = [
            ...
            for k in range(num_tower)]
        all_results = multithread_predict_dataflow(dataflows, graph_funcs)
        output = os.path.join(os.path.dirname(os.path.abspath(__file__)), output_file)
        DatasetRegistry.get(dataset).eval_inference_results(all_results, output)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test', help="Run test. "
                                           "This argument is the path to the output json test file")
    ...
    register_obj365(cfg.DATA.BASEDIR)  # add COCO datasets to the registry
    ...
    if args.visualize:
        ...
        if args.predict:
            ...
            assert args.test.endswith('.json'), args.test
